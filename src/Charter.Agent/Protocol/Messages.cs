...
/// <remarks>
/// The pairing token is single-use and short-TTL. It is spent here and never written to disk; what
/// the agent keeps is the long-lived credential returned in <see cref="PairResponse"/>.
/// </remarks>
...
public sealed record PairResponse
{
    ...
    /// <summary>The long-lived agent credential. Stored 0600 on the agent host; never logged.</summary>
...
public sealed record HelloPayload
{
    ...
    /// <summary>Every version this build can speak, newest first. Lets the plane pick a common one.</summary>
...
public sealed record WelcomePayload
{
    ...
    /// <summary>Every version the plane can speak. Purely informational for the agent's error text.</summary>
...
{
    /// <summary>Never more than the remaining concurrency slots.</summary>
...
public sealed record JobAssignment
{
    ...
    /// <summary>Matches the control plane's <c>JobType</c>: <c>recon</c>, <c>build</c>, <c>recap</c>, and so on.</summary>
...
/// </summary>
/// <remarks>
/// A GitHub installation token scoped to the one repository in this job, and a scoped model
...
/// this host and never transmitted (section 32.8).
/// <para>
/// <see cref="ToString"/> is overridden because the default record printer would put every secret
/// value into any interpolated string that touched one.
/// </para>
/// </remarks>
...
/// <summary>Streamed progress for a running job. Output is scrubbed before it is sent.</summary>
